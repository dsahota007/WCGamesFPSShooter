...
public class EnemyHealthNoRagdoll : MonoBehaviour
{
    public int Health = 100;

    // public GameObject ragdollRoot;            // << RAGDOLL: not used in this no-ragdoll version
    // public float ragdollForce = 3f;           // << RAGDOLL: not used
    ...
    // ===== NEW: instant death VFX + despawn =========================
    [Header("No-Ragdoll Death")]
    [Tooltip("Spawned immediately on death at this transform, then we despawn shortly after.")]
    public GameObject deathExplosionVFX;        // << NEW
    public Vector3 deathVFXOffset = Vector3.up; // << NEW
    public float deathDestroyDelay = 0.35f;     // << NEW
    // ================================================================
    ...
    void Start()
    {
        ...
        // SetRagdollState(false);                // << RAGDOLL: not used in this version
    ...
    {
        // if (ragdollRoot == null) { return; }   // << RAGDOLL: not relevant

        isDead = true;
        PointManager.Instance?.AddPoints(isMiniboss ? minibossPoints : 50);
        ...
        // --------- NEW: play death explosion VFX immediately ----------
        if (deathExplosionVFX != null)
        {
            var fx = Instantiate(deathExplosionVFX, transform.position + deathVFXOffset, Quaternion.identity);
            Destroy(fx, 10f); // cleanup VFX
        }
        // --------------------------------------------------------------

        // (in the ragdoll version we would: SetRagdollState(true); ApplyRagdollForce(hitDirection);)
        // Change layer to DeadBody (no collision with player) — not needed if we despawn fast

        GameObject player = GameObject.FindGameObjectWithTag("Player");
        if (player != null)
        {
            foreach (var enemyCol in GetComponentsInChildren<Collider>())
            {
        ...
        // instantly (or shortly) despawn the whole enemy
